...
const Userstatus = ({ dorname }) => {
  ...
  useEffect(() => {
    if (username) {
      fetch(
        "https://dormatery-project-default-rtdb.firebaseio.com/conformdetails.json"
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (username) {
      fetch(
        "https://dormatery-project-default-rtdb.firebaseio.com/ratingreview.json"
      )
        .then((res) => res.json())
        .then((newdata) => {
          const objnewdata = Object.values(newdata).filter(
      ...
      >
        ...
        >
          <TabList>
            ...
            <Tab
              sx={{ mt: "5px", fontFamily: "Josefin Sans", fontWeight: "bold" }}
            >
              Rating & Review
            </Tab>
          </TabList>
          ...
          <TabPanel value={1}>
            <FormControl
              variant="standard"
              sx={{ m: 1, minWidth: 200, marginBottom: "20px" }}
            >
